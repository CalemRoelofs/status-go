package controllers

import (
	"crypto/ecdsa"

	"github.com/golang/protobuf/proto"
	"go.uber.org/zap"

	"github.com/status-im/status-go/crypto"
	cryptotypes "github.com/status-im/status-go/crypto/types"
	ethtypes "github.com/status-im/status-go/eth-node/types"
	"github.com/status-im/status-go/messaging/layers/encryption"
	"github.com/status-im/status-go/messaging/layers/reliability"
	"github.com/status-im/status-go/messaging/layers/segmentation"
	"github.com/status-im/status-go/messaging/layers/transport"
	"github.com/status-im/status-go/pkg/pubsub"
)

type Sender struct {
	identity *ecdsa.PrivateKey

	transport    *transport.Transport
	segmentation *segmentation.Segmenter
	encryption   *encryption.Protocol
	reliability  *reliability.Reliability

	logger *zap.Logger

	publisher *pubsub.Publisher
}

func NewSender(
	identity *ecdsa.PrivateKey,
	transport *transport.Transport,
	segmentation *segmentation.Segmenter,
	encryption *encryption.Protocol,
	reliability *reliability.Reliability,
	logger *zap.Logger,
) *Sender {
	return &Sender{
		identity:     identity,
		transport:    transport,
		segmentation: segmentation,
		encryption:   encryption,
		reliability:  reliability,
		logger:       logger.Named("sender"),
		publisher:    pubsub.NewPublisher(),
	}
}

func (s *Sender) Start() error {
	return s.StartReliability()
}

func (s *Sender) Stop() {
	s.StopReliability()
}

func (s *Sender) Publisher() *pubsub.Publisher {
	return s.publisher
}

func messageID(sender *ecdsa.PublicKey, payload []byte) cryptotypes.HexBytes {
	keyBytes := crypto.FromECDSAPub(sender)
	return cryptotypes.HexBytes(crypto.Keccak256(append(keyBytes, payload...)))
}

func (s *Sender) processAndMarshalMessageSpec(spec *encryption.ProtocolMessageSpec) ([]byte, []byte, error) {
	// The shared secret needs to be handle before we send a message
	// otherwise the topic might not be set up before we receive a message
	if spec.SharedSecret != nil {
		_, err := s.transport.ProcessNegotiatedSecret(ethtypes.NegotiatedSecret{
			PublicKey: spec.SharedSecret.Identity,
			Key:       spec.SharedSecret.Key,
		})
		if err != nil {
			return nil, nil, err
		}
	}

	var sharedSecretKey []byte
	if spec.AgreedSecret {
		sharedSecretKey = spec.SharedSecret.Key
	}

	messageBytes, err := proto.Marshal(spec.Message)
	if err != nil {
		return nil, nil, err
	}

	return messageBytes, sharedSecretKey, nil
}
