...
func validateMessageID(messageID string, rawMessage *RawMessage) error {
...
func setMessageID(messageID cryptotypes.HexBytes, rawMessage *RawMessage) error {
	...
	if err := validateMessageID(messageIDString, rawMessage); err != nil {
		return err
	}
...
func (s *MessageSender) notifyOnScheduledMessage(recipient *ecdsa.PublicKey, message *RawMessage) {
	pubsub.Publish(s.publisher, ScheduledMessageEvent{
		Recipient:  recipient,
		RawMessage: message,
	})
}
