...
func mapTokenGroupKeysToIDs(tokenGroupKeys []string, tokenIdMap map[string]GeckoToken) (mappedKeys map[string]bool, uniqueIDs []string) {
	mappedKeys = make(map[string]bool, 0)
	uniqueIDsMap := make(map[string]struct{}, 0)
	for _, gtKey := range tokenGroupKeys {
...
func (c *Client) FetchPrices(tokenGroupKeys []string, currencies []string) (map[string]map[string]float64, error) {
	if len(tokenGroupKeys) == 0 {
	...
	mappedKeys, uniqueIDs := mapTokenGroupKeysToIDs(tokenGroupKeys, tokenIDMap)
...
func (c *Client) FetchTokenDetails(tokenGroupKeys []string) (map[string]thirdparty.TokenDetails, error) {
	if len(tokenGroupKeys) == 0 {
		return nil, fmt.Errorf("no tokens provided")
	}
	tokenIDMap, err := c.getTokenIDTokenMap()
	...
	mappedKeys, _ := mapTokenGroupKeysToIDs(tokenGroupKeys, tokenIDMap)
...
func (c *Client) FetchTokenMarketValues(tokenGroupKeys []string, currency string) (map[string]thirdparty.TokenMarketValues, error) {
	if len(tokenGroupKeys) == 0 {
		return nil, fmt.Errorf("no tokens provided")
	}
	if currency == "" {
...
func (c *Client) FetchHistoricalHourlyPrices(tokenGroupKey string, currency string, limit int, aggregate int) ([]thirdparty.HistoricalPrice, error) {
...
func (c *Client) FetchHistoricalDailyPrices(tokenGroupKey string, currency string, limit int, allData bool, aggregate int) ([]thirdparty.HistoricalPrice, error) {
	if tokenGroupKey == "" {
	...
	_, uniqueIDs := mapTokenGroupKeysToIDs([]string{tokenGroupKey}, tokenIDMap)
