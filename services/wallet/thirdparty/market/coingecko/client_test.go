...
const (
	...
	statusGroupTokenKey      = "status"
	unsupprotedGroupTokenKey = "UNSUPPORTED"
	tokensGroupTokenKey      = "TOKENS"
)
...
func TestFetchPrices(t *testing.T) {
	tokenGroupKeys := []string{
		ethGroupTokenKey,
		statusGroupTokenKey,
		unsupprotedGroupTokenKey,
		tokensGroupTokenKey,
	}

	var expectedPrices = make(map[string]map[string]float64)
	expectedPrices[ethGroupTokenKey] = make(map[string]float64)
	expectedPrices[ethGroupTokenKey]["USD"] = 3181.32
	expectedPrices[statusGroupTokenKey] = make(map[string]float64)
	expectedPrices[statusGroupTokenKey]["USD"] = 0.02391704
	expectedPrices[unsupprotedGroupTokenKey] = make(map[string]float64)
	expectedPrices[unsupprotedGroupTokenKey]["USD"] = 0
	expectedPrices[tokensGroupTokenKey] = make(map[string]float64)
	expectedPrices[tokensGroupTokenKey]["USD"] = 0
	...
	prices, err := geckoClient.FetchPrices(tokenGroupKeys, []string{"USD"})
	require.NoError(t, err)
...
func TestFetchMarketValues(t *testing.T) {
	...
	expectedMarketValues[ethGroupTokenKey] = thirdparty.TokenMarketValues{
	...
	expectedMarketValues[statusGroupTokenKey] = thirdparty.TokenMarketValues{
	...
	expectedMarketValues[unsupprotedGroupTokenKey] = thirdparty.TokenMarketValues{}
	expectedMarketValues[tokensGroupTokenKey] = thirdparty.TokenMarketValues{}
	...
	prices, err := geckoClient.FetchTokenMarketValues(tokenGroupKeys, "USD")
	require.NoError(t, err)
